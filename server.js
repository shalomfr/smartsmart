  ...
  imap.once('ready', () => {
    ...
    imap.openBox(folderName, true, (err, box) => {
      ...
      const fetch = imap.seq.fetch(`${Math.max(1, box.messages.total - 20)}:*`, {
        bodies: ['HEADER', 'TEXT'],
        struct: true,
        envelope: true // כולל מעטפה עם מידע נוסף
      });
          ...
          msg.once('attributes', (attrs) => {
            ...
            if (attrs['x-gm-labels']) {
              email.labels = attrs['x-gm-labels']
                .filter(label => {
                  // סנן תוויות עם תווים מוזרים או קודים
                  return !label.startsWith('-') && !label.includes('&') && label.length < 50;
                })
                .map(label => {
                  // המר תוויות של Gmail לעברית (case insensitive)
                  // נקה סלאש מהסוף
                  if (label.endsWith('\\')) {
                  ...
                  // חפש תרגום - קודם לפי uppercase, אחר כך לפי המקורי
                  return labelMap[upperLabel] || labelMap[label] || 
                         (label.length > 15 ? null : label); // הסתר תוויות ארוכות מדי
                })
                .filter(label => label !== null); // הסר תוויות שסוננו
            } else {
              // עבור ספקי דוא"ל אחרים, השתמש בתיקיות
...
חשוב מאוד: פרק את התשובה לפיסקאות נפרדות עם שורות ריקות ביניהן. כל רעיון או נושא אמור להיות בפיסקה נפרדת.

דוגמה לפורמט נכון:
שלום [שם],

תודה לך על פנייתך בנושא...

בנוגע לשאלתך, אני רוצה לענות...
